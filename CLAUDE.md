...
- **A column type change is not idempotent unless you guard it.** `ALTER COLUMN ... TYPE` rewrites the whole table under ACCESS EXCLUSIVE *every* time it runs, which is the same boot-time deadlock the credits CHECK constraint was rewritten to avoid. The credit columns' widening to `NUMERIC(12, 2)` is wrapped in a check against `information_schema.columns`, so the second and every later boot does nothing. Verified by re-running `schema.sql` and confirming the tables' filenodes are unchanged.
