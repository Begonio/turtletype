...
const AUTH_ERRORS: Record<string, string> = {
  ...
  access_denied: 'You declined the permissions HumanType needs to write to your document.',
      ...
      <main className="mx-auto max-w-5xl px-6">
        <section className="pt-16 pb-20 sm:pt-24">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-ink-300">
            HumanType types your text into a Google Doc character by character — variable speed,
            pauses at punctuation, the occasional typo caught three letters later. Paste it, pick a
            pace, and watch the document fill in.
          </p>
          ...
          <p className="mt-4 max-w-xl text-xs leading-relaxed text-ink-400">
            HumanType asks for permission to edit Google Docs so it can write into the document you
...
{`The quick brown fox jumsp`}<span className="text-red-400">{`s`}</span>{` — `}
<span className="text-ink-400">{`(backspace ×3)`}</span>{`
The quick brown fox jumps over the lazy dog,`}
<span className="text-ink-400">{`   ← 280ms pause`}</span>
{`
 and it does so repeatedly.`}<span className="caret text-accent-500">▌</span>
          </pre>
        </section>
      </main>
      ...
      <footer className="border-t border-ink-800 py-8">
        <div className="mx-auto max-w-5xl px-6 text-xs text-ink-400">
          HumanType — built for people who want their words to land like they were written, not
          pasted.
        </div>
      </footer>
    </div>
